...
typedef struct pkb_node_t {
	fmpz_poly_mat_t pkb;
	struct pkb_node_t *next;
} pkb_node_t;

int main(int argc, char *args[])/* params, ct1.txt, ct2.txt, pk.txt */
{
	FILE *fp;
        ...
        pkb_node_t *pktmp, *pks, *pkr;
        long ii, jj;
        pktmp = (pkb_node_t *)malloc(sizeof(pkb_node_t));
        
        pks = pktmp;
        for(i=0;i<=lev;i++) {
                pkr = (pkb_node_t *)malloc(sizeof(pkb_node_t));
                fgets(str, 30, fp);
                fmpz_set_str(tmp, str,10);
                row = fmpz_get_si(tmp);
        ...
        while( l >= low ) {
                ...
                hcrypt_bgv_refresh(ctmp, tmpp, pktmp->pkb, hp->q, hp->next->q, fx, d);
        ...
        fmpz_poly_mat_t c3, c4, ct;
        long row1 = fmpz_poly_mat_nrows(levhigh), row2 = fmpz_poly_mat_nrows(levlow);
        if(row1 > row2)
                row = row1;
        else
                row = row2;
        ...
        hcrypt_bgv_refresh(ct, c3, pktmp->pkb, hp->q, hp->next->q, fx, d);
        printf("%ld\n%ld\n1\n", ctlev, row);
        
        for( i = 0; i < row; i++) {
                fmpz_poly_print(fmpz_poly_mat_entry(ct, i, 0));
                printf("\n");
        }
